...
function eaeDecode(encoded) {
  // console.log('eaeDecode Ran with encoded payload', encoded, "type: ", typeof(encoded))
  var data = base58.decode(encoded);
  // console.log('decoded data was ', data, "type: ",  typeof(data), "length:", data.length, data[0], data[1], data[2], data[3])
  var netLength = data.length - 24;
  // console.log('netLength ', netLength, "type: ",  typeof(netLength))
  var network = data.slice(1, netLength);
  // console.log('network ', network, "type: ",  typeof(network), "************dump: ", network[0], network[1], network[2] )
  var address = data.slice(netLength, 20 + netLength);
  // console.log('address ', address, "type: ",  typeof(address), "************dump: ", address[0], address[1], address[2])
  var pppp = {
...
function UportLite() {
  ...
  function pad(pad, str, padLeft) {
    if (typeof str === 'undefined') {
      // console.log('str undefined, pad: ' + pad)
      return pad;

    }
    if (padLeft) {
      // console.log('padLeft', (pad + str), (pad + str).slice(-pad.length))
      return (pad + str).slice(-pad.length);
    } else {
      // console.log('not padLeft', (str + pad), pad.length, (str + pad).substring(0, pad.length))
      return (str + pad).substring(0, pad.length);
    }
  }

  function encodeFunctionCall(functionSignature, registrationIdentifier, issuer, subject) {
    // console.log("Encoding Function Call", functionSignature, registrationIdentifier, issuer, subject)
    // console.log('********************** Stubs:', asciiToHex(registrationIdentifier), issuer.slice(2), subject.slice(2))
  ...
  function callRegistry(registrationIdentifier, issuerId, subjectId, callback) {
    // console.log('callRegistry Triggered', registrationIdentifier, issuerId, subjectId, callback)
